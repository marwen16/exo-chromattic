...
@ComponentConfig(
	    ...
	    template = "classpath:groovy/webui/UIGalleryViewerConfig.gtmpl",
	    events = {
...
public class UIGalleryViewerConfig extends UIForm implements UISelectable {
	...
	public static class SelectFolderPathActionListener extends EventListener<UIGalleryViewerConfig> {
		...
		@Override
		public void execute(Event<UIGalleryViewerConfig> event)
				throws Exception {
			UIGalleryViewerConfig uiUpload = event.getSource();
            UIContentSelectorFolder contentSelector = uiUpload.createUIComponent(UIContentSelectorFolder.class, null, null);    
            contentSelector.init("collaboration", "/");
            contentSelector.getChild(UIContentBrowsePanelFolder.class).setSourceComponent(uiUpload, new String[] { "path" });
            Utils.createPopupWindow(uiUpload, contentSelector, UIContentSelector.CORRECT_CONTENT_SELECTOR_POPUP_WINDOW, 800);
